...
class Subject(db.Model):
    ...
    public_key_path = db.Column(db.String(150), nullable=False)
...
class ACL(db.Model):
    ...
    role = db.relationship("Role", back_populates="acl_entries")
...
class Role(db.Model):
    ...
    acl_entries = db.relationship("ACL", back_populates="role", cascade="all, delete-orphan")
...
class Session(db.Model):
    ...
    roles = db.relationship(
        "Role",
        secondary="subject_role",
        primaryjoin="Session.subject_id == SubjectRole.subject_id",
        secondaryjoin="SubjectRole.role_id == Role.id",
        viewonly=True
    )
